...
class Prioritized_Replaybuffer():
    ...
    def __init__(self,capacity):
        self.not_full_use_buff=ReplayMemory(capacity)
    ...
    def appendnew(self,lastobs,act,state,reward):
        if self.full==0:
            ...
            if self.recordinglength>self.maxlen:
                self.full=1
        ...
        if self.full==0:
            self.not_full_use_buff.memory.append(transition)
    ...
    # 此处是计算损失函数的过程，可见是要将整个loss都乘以权重，换言之，操作只在loss计算中进行
            ...
            self.weight_record=torch.ones(BATCH_SIZE,1).to(self.device)
            self.index_recorded=0
            state_batch,action_batch,reward_batch,non_final_mask,non_final_next_states=self.not_full_use_buff.get_Batch_data(BATCH_SIZE)
            return state_batch,action_batch,reward_batch,non_final_mask,non_final_next_states
